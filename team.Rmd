...
defenseGroup %>% 
  ggplot(aes(Total)) + geom_histogram(bins = 25) + facet_wrap(~Statistic) + labs(title = "Histogram of the total number of each Defensive Statistic for each player during the season")
```
...
defGather %>%
  ...
  ggplot(aes(x = `West Virginia`, y = `Kansas State`)) + geom_point() + labs(title = "Solo Tackles in West Virginia game vs Solo Tackles in Kansas State game")
```
...
offBios %>% group_by(Weight) %>% summarize(Receiving_YDS = sum(Receiving_YDS)) %>% ggplot(aes(Weight, Receiving_YDS)) + geom_point() + labs(title = "The weight of a player vs their receiving yards from the year")
```
...
allTrim %>% filter(Position == c("Wide Receiver", "Tight End", "Quarterback", "Running Back", "Defensive Back", "Linebacker", "Cornerback")) %>% group_by(Position) %>% summarize(meanWeight = mean(Weight)) %>% ggplot(aes(x = Position, y = meanWeight)) + geom_bar(stat = "identity")
```
